...
get.next.manoc<-function(y,n,target,p.sample.mat,delta,j_curr,k_curr,alpha,eta)
{
	...
	pos.model[which(n==0,arr.ind=TRUE)]<-pos.model[which(n==0,arr.ind=TRUE)]+delta
	...
	{		
		...
		if (max(p.rgn)<eta)
		{
			...
			action<-which(l==min(l)) # action==1-->De-escalation. action==2-->Retainment. action==3-->Escalation.
